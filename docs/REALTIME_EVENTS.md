...
- POST /devices adds optional appRole (same role enum, membership validated) and tokenProvider=fcm|apns. Omitting tokenProvider preserves FCM routing on every platform, including legacy iOS registrations. Native APNs requires explicit tokenProvider=apns on an iOS registration. Anonymous POST /devices accepts the same provider selection and retains its fixed success body. Existing clients may omit appRole. All registrations remain single-account owned.
