...
| 4 | Restored | Existing restore decision durable; only actual decided membership/case restores; identity/dispatch/services refresh. A restore that also brings lines back (`restoreServiceIds`) sends the supplier one "Your shop is back on GRIDGO" notice and suppresses the per-line supplier service notice; Operations keep per-line copies. |
