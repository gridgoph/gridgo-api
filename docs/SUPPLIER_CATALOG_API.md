...
Service lines use `draft` while incomplete and `pending_verification` when review-ready. Both self-service state changes and compatibility submit/verify actions reject incomplete lines with `service_not_review_ready`. Independently suspended and withdrawn complete lines may be resubmitted to `pending_verification`; suppliers cannot make a line live. Lines suspended by an account approval case remain under that case's restore boundary: suppliers may make safe remediation edits but cannot submit, withdraw, or otherwise change their lifecycle state. A live line returns to `pending_verification` when the supplier expands its governed category or accepted formats. Routine catalog, price, and option edits remain immediately derived from approval + live-service eligibility.
