...
Service lines use `draft` while incomplete and `pending_verification` when review-ready. Both self-service state changes and compatibility submit/verify actions reject incomplete lines with `service_not_review_ready`. Independently suspended and withdrawn complete lines may be resubmitted to `pending_verification` only while the supplier account is approved; suppliers cannot make a line live. While an account approval case is suspended, suppliers may edit catalog and settings content needed for remediation but cannot submit, withdraw, or otherwise change any line's lifecycle state. `/auth/me/supplier` exposes that subset as `editCatalogue: true`, `editSettings: true`, `changeServiceLifecycle: false`, and `publishServiceLines: false`; matching and job capabilities remain false. A live line returns to `pending_verification` when the supplier expands its governed category or accepted formats. Routine catalog, price, and option edits remain immediately derived from approval + live-service eligibility.
