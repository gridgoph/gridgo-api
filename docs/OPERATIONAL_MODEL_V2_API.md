...
Initial supplier approval requires the complete catalog readiness contract: shop/contact/location, payment terms, shop identity media, and at least one complete active item under an eligible `pending_verification` service line. Eligible pending lines publish to `live` in the approval transaction; incomplete lines remain pending. Failure returns `409 supplier_profile_incomplete` with `missing`. Supplier suspension is never blocked by readiness, records each live line's prior state, advances its optimistic version, and makes it `suspended`. Restore consumes the full readiness contract and republishes only eligible lines suspended by that account case; independently suspended lines still require explicit resubmission and review through `/supplier-services/:id/verify`.
