...
- Fixed application routes add only the membership named by the URL: `/auth/clerk/enroll/supplier`, `/auth/clerk/enroll/rider`, and `/me/business-application`. They require `Idempotency-Key`; supplier/business submit immediately, while the rider projection resumes document intake and explicit submission after a current licence is attached.
