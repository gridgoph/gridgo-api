...
Every merge to the default branch ships: `.github/workflows/deploy.yml` builds `Dockerfile`, smoke tests the image, publishes to GHCR as `${REGISTRY}/${GITHUB_REPOSITORY,,}` (now `ghcr.io/gridgoph/gridgo-api`), then runs the one restricted command the deploy key allows (`ssh … api`, registry token on stdin). Pull requests build but never publish and never deploy. `deploy/docker-compose.yml` hardcodes that same image and is the server's copy at `~/gridgo/api/docker-compose.yml`; **CI never writes to the server**, so changing it here requires an operator to copy it across. The personal-account GHCR package from before the 2026-08-15 org transfer did not move with the repo and is orphaned.
