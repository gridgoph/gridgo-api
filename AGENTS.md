...
- Checkout is 100% up front by default: the setting `downpaymentPercent` (100 or 75) is snapshotted per order, like the rider split. A zero balance stays in the installment list as `final_online` with status `not_required`; balance gates must use `paymentSettled` (`src/operational-model.js`), never `status === "confirmed"`. Orders without the snapshot are legacy 75/25. Contract: `docs/OPERATIONAL_MODEL_V2_API.md#upfront-checkout`.
- Supplier payout stages are snapshotted per order as `payoutPlanVersion` (1 = legacy 50/15/25/10, 2 = escrow 40/35/25 of the shop's own cost). Shares live only in `src/payout-plan.js`, mirrored per version by SQL `validate_order_payout_stages`; a new split is a new version plus a forward migration, never an edit to a published plan. Only Operations/Super Admin release plan-2 stages; the client's confirm only closes the window. Contract: `docs/OPERATIONAL_MODEL_V2_API.md#supplier-payout-milestones`.
...
- Public landing support tickets live in `support_admins` / `support_tickets` (`src/support-desk.js`). They use `DATABASE_URL` through `database.query` / transactions, not a second Pool. Desk access (tickets and `/issue-reports`) is a Clerk session whose verified primary email is on `SUPPORT_DESK_ALLOWED_EMAILS` (`deskOperatorFromRequest`); `POST /admin/login` is retired and the landing origin must be a Clerk authorized party. Gmail replies are optional via `EMAIL_USER` / `EMAIL_PASSWORD`.
- The Super Admin Tracker (`src/tracker.js`, contract `docs/TRACKER_API.md`) reads and writes GitHub issues labelled `tracker` over plain `fetch` with the server-only `GITHUB_TRACKER_TOKEN`; GitHub stays the status source of truth, PostgreSQL keeps only `tracker_decisions`. Unset token = `503 tracker_not_configured`; unset `FIRSTMATE_TRACKER_TOKEN` = firstmate routes `404`. Its writes hold the `gridgo-tracker` lock during GitHub calls and take the domain lock only for the final audit commit. Tests mock GitHub via `GITHUB_TRACKER_API_URL`; never call the real API.
- Public issue reports from the landing `/report` page live in `issue_reports` / `issue_report_screenshots` (`src/issue-reports.js`, contract `docs/ISSUE_REPORTS_API.md`). Filing is public; reading and marking use the Clerk support desk gate (`/issue-reports`) or an Operations / Super Admin Clerk session (`/ops/issue-reports`, the dashboard); firstmate uses `/firstmate/issue-reports` with the `FIRSTMATE_TRACKER_TOKEN` bearer to mark reports `tracked` with a `trackerIssueUrl` (it files the GitHub issue itself).
