...
/**
 * Written inside the domain mutation, after notification rows exist. No token
 * material is copied. Returns how many outbox rows were written so `save()`
 * knows whether a post-commit drain is worth kicking.
 */
...
/**
 * Multi-process lease; a crash is retried after the lease, bounded
 * at-least-once transport.
 *
 * `drain()` is single-flight per process. A call that lands while a drain is
 * running returns at once and asks the running drain to make one more pass
 * when it finishes, so a row committed after the running drain's last claim is
 * still sent immediately instead of waiting for the next lifecycle tick. The
 * SQL lease (`status='sending'`, `FOR UPDATE SKIP LOCKED`) keeps other
...
export function createOutboxWorker({
  ...
  async function drainOnce() {
    ...
    async function processRow(row, store) {
    ...
    while (true) {
      ...
      // One snapshot per claimed batch, read after the claim so ownership,
      // membership and deletion rechecks still see every commit that
      // preceded this attempt.
      const store = await loadStore(database);
      let next = 0;
      async function worker() {
        while (next < claimed.rows.length) {
          const row = claimed.rows[next++];
          await processRow(row, store);
        }
      }
