...
export async function loadStore(database) {
  ...
  store.notifications = ordered(loaded.notifications).map((row) => { const item = { ...row.data, id: row.id, userId: row.user_id, type: row.type, title: row.data.title, body: row.data.body, read: row.read_at != null, at: row.created_at }; delete item.title; delete item.body; Object.assign(item, row.data); if (row.order_id != null) item.orderId = row.order_id; if (row.deleted_at != null) item.deletedAt = row.deleted_at; return item; });
